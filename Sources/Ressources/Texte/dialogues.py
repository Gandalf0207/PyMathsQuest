from settings import *

DialoguesFr = {
    "NiveauPlaineRiviere" : {
        "Seconde" : {
            "Numero0": {
                "PNJ1": {
                    "Nom" : "Bûcheron",
                    "Principal": {
                        "Dialogue1": "Bonjour toué ! Dis donc tu m’as l’air de frapper ton waterloo toi ! Mais lâche pas la patate je vais t’aider, cet arbre va pas te bloquer le chemin longtemps tabarnak !",
                        "Dialogue2": "Aussi prends c’te babiole ça pourrait t’aider t’sais, il y a écrit un charabia dessus j’te la laisse.",
                    },
                    "Alternatif": {
                        "Dialogue1": "Alors, fonce l’ami n’ai pas peur de l’eau sous le tronc elle te sautera pas dessus !"
                    }
                },
                "PNJ2": {
                    "Nom" : "Voyageur curieux",
                    "Principal": {
                        "Dialogue1": "Salut l’ami si tu souhaites traverser l’Anduin il te faudra créer un pont je n’ai rien d’autre que ces planches alors je vais te les donner et j'espère que tu en feras bon usage.",
                        "Dialogue2": "J’ai même un manuel mais je ne sais pas s’il te sera utile.",
                    },
                    "Alternatif": {
                        "Dialogue1": "Alors que trouve-t-on de l’autre côté ? Tu me raconteras une fois que tu l’auras vu."
                    }
                },
                "PNJ3": {
                    "Nom" : "Âne fuyard",
                    "Principal": {
                        "Dialogue1": "(Il semble que cet âne se soit enfui, il y a probablement une ferme à proximité. Il a laissé tomber deux objets, on dirait un parchemin et une pioche.)",
                    },
                    "Alternatif": {
                        "Dialogue1": "(Cet âne n’a pas l’air de vouloir rentrer chez lui)"
                    }
                }
            },
        },

    },

    "NiveauMedievale" : {
        "Seconde" : {
            "Numero0": {
                "PNJ1": {
                    "Nom" : "Bûcheron",
                    "Principal": {
                        "Dialogue1": "Bonjour toué ! Dis donc tu m’as l’air de frapper ton waterloo toi ! Mais lâche pas la patate je vais t’aider, cet arbre va pas te bloquer le chemin longtemps tabarnak !",
                        "Dialogue2": "Aussi prends c’te babiole ça pourrait t’aider t’sais, il y a écrit un charabia dessus j’te la laisse.",
                    },
                    "Alternatif": {
                        "Dialogue1": "Alors, fonce l’ami n’ai pas peur de l’eau sous le tronc elle te sautera pas dessus !"
                    }
                },
                "PNJ2": {
                    "Nom" : "Voyageur curieux",
                    "Principal": {
                        "Dialogue1": "Salut l’ami si tu souhaites traverser l’Anduin il te faudra créer un pont je n’ai rien d’autre que ces planches alors je vais te les donner et j'espère que tu en feras bon usage.",
                        "Dialogue2": "J’ai même un manuel mais je ne sais pas s’il te sera utile.",
                    },
                    "Alternatif": {
                        "Dialogue1": "Alors que trouve-t-on de l’autre côté ? Tu me raconteras une fois que tu l’auras vu."
                    }
                },
                "PNJ3": {
                    "Nom" : "Âne fuyard",
                    "Principal": {
                        "Dialogue1": "(Il semble que cet âne se soit enfui, il y a probablement une ferme à proximité. Il a laissé tomber deux objets, on dirait un parchemin et une pioche.)",
                    },
                    "Alternatif": {
                        "Dialogue1": "(Cet âne n’a pas l’air de vouloir rentrer chez lui)"
                    }
                },
                "PNJ4": {
                    "Nom" : "Âne fuyard",
                    "Principal": {
                        "Dialogue1": "(Il semble que cet âne se soit enfui, il y a probablement une ferme à proximité. Il a laissé tomber deux objets, on dirait un parchemin et une pioche.)",
                    },
                    "Alternatif": {
                        "Dialogue1": "(Cet âne n’a pas l’air de vouloir rentrer chez lui)"
                    }
                },
            },
        },

    },

}




































DialoguesEn = {
    "Niveau0": {
        "PNJ1": {
            "Nom" : "Lumber",
            "Principal": {
                "Dialogue1": "Hello there! Well, you look like you're hitting a wall, huh! But don't give up, I'll help you out. This tree won't block your way for long !",
                "Dialogue2": "Also, take this scroll, it might help you, you know. There's some gibberish written on it, so I'll leave it to you",
            },
            "Alternatif": {
                "Dialogue1": "So, go ahead bud, don’t be afraid of the water under the tree it won’t jump on you !"
            }
        },
        "PNJ2": {
            "Nom" : "Curious traveller",
            "Principal": {
                "Dialogue1": "Hello, my friend! If you wish to cross the Anduin, you'll need to build a bridge. I don't have much except these planks, so I'll give them to you, and I hope you'll put them to good use.",
                "Dialogue2": "I even have a manual, though I'm not sure if it will be helpful to you.",
            },
            "Alternatif": {
                "Dialogue1": "So what’s on the other side ? Tell me once you’ve saw it."
            }
        },
        "PNJ3": {
            "Nom" : "Fugitive Donkey",
            "Principal": {
                "Dialogue1": "(It seems that this donkey has run off; there's probably a farm nearby. It dropped two items, which look like a scroll and a pickaxe.)",
            },
            "Alternatif": {
                "Dialogue1": "(It doesn’t seem like this donkey wants to go back home.)"
            }
        }
    },






    "Niveau1": {
        "PNJ1": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        },
        "PNJ2": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        },
        "PNJ3": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        }
    },
    "Niveau3": {
        "PNJ1": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        },
        "PNJ2": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        },
        "PNJ3": {
            "Nom" : "Abrakadabra",
            "Principal": {
                "Dialogue1": "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.",
                "Dialogue2": "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur",
                "Dialogue3": "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum."
            },
            "Alternatif": {
                "Dialogue1": "deja vu"
            }
        }
    }
}


